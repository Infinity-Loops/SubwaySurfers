...
public class OnlineSettings
{
	...
	public void DownloadNow()
	{
		if (!_isDownloading)
		{
			...
			_downloadGameObject.AddComponent<Empty>().StartCoroutine(DownloadCoroutine());
		}
	}
	...
	{
		WWW www = new WWW("http://hoodrunner.kiloo.com/onlinesettings.php?android");
		yield return www;
		if (www.error == null)
		{
			string response = www.text;
			if (!response.Contains("<html") && ValidateChecksum(response))
			{
