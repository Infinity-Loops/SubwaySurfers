...
public class InAppManager : MonoBehaviour
{
	...
	public void PurchaseSuccess(string productId)
	{
		...
		FlurryAndroid.logEvent("InApp purchase completed", dictionary);
		dictionary.Clear();
		if (productId == InAppData.inAppTier1)
		{
			dictionary.Add("Mission Set", PlayerInfo.Instance.currentMissionSet.ToString());
			FlurryAndroid.logEvent("InApp Coin Pack 1 purchased", dictionary);
		}
		else if (productId == InAppData.inAppTier2)
		{
			dictionary.Add("Mission Set", PlayerInfo.Instance.currentMissionSet.ToString());
			FlurryAndroid.logEvent("InApp Coin Pack 2 purchased", dictionary);
		}
		else if (productId == InAppData.inAppTier3)
		{
			dictionary.Add("Mission Set", PlayerInfo.Instance.currentMissionSet.ToString());
			FlurryAndroid.logEvent("InApp Coin Pack 3 purchased", dictionary);
		}
	}
