...
public class UIScreenController : MonoBehaviour
{
	...
	private void onDestroy()
	{
		...
		SocialManager.instance.RemoveFriendsConsolidatedHandler(CheckForLoginBonus);
	}
	...
	public void CheckForLoginBonus()
	{
		if (SocialManager.instance.facebookIsLoggedIn && !PlayerInfo.Instance.hasPayedOutFacebook)
		{
			...
			PlayerInfo.Instance.SaveIfDirty();
			Instance.QueueFacebookPayoutPopup();
		}
		if (Social.localUser.authenticated && !PlayerInfo.Instance.hasPayedOutGameCenter)
		{
			...
			PlayerInfo.Instance.SaveIfDirty();
			Instance.QueueGameCenterPayoutPopup();
		}
	}
	...
	private void ScreenDidChange(string newScreenName)
	{
		...
		FlurryAndroid.logEvent("UI Screen " + newScreenName);
	...
	private void OnTokenPickUp(CharacterModels.TokenType type)
	{
		foreach (int value in Enum.GetValues(typeof(CharacterModels.ModelType)))
		{
			...
			if (flag && PlayerInfo.Instance.GetCollectedTokens((CharacterModels.ModelType)value) == model.Price)
			{
				...
				FlurryAndroid.logEvent("Character unlocked", dictionary);
			}
		}
	}
